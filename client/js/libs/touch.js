...
utils.getXYByElement = function(el) {
    ...
    while (el.offsetParent) {
        left += el.offsetLeft;
        top += el.offsetTop;
        el = el.offsetParent;//offsetParent返回一个指向最近的（closest，指包含层级上的最近）包含该元素的定位元素。
    }
    return {
        left: left,
        top: top
...
var gestures = {
    ...
    rotateSingleFinger: function(ev) {
        //获取触摸的元素
        var el = ev.target;
        if (__rotation_single_finger && utils.getFingers(ev) < 2) {//再次判断是否是单指
            //判断单手指有没有移动，若没有移动，直接返回
            if (!pos.move) return;
            //
...
var handlerOriginEvent = function(ev) {
    ...
    switch (ev.type) {
        ...
        case 'mousemove':
            ...
            //将touch点的pageX、pageY信息存入pos.move
...
var _on = function() {
    ...
    if (args.length === 3 && utils.getType(args[1]) === 'string') {
        ...
        evts.forEach(function(evt) {
            if (!utils.hasTouch) {//若是PC端
                evt = utils.getPCevts(evt);
            }
            els.forEach(function(el) {
                //为每个目标元素绑定事件，以及事件处理函数
...
var _dispatch = function(el, evt, detail) {
    ...
    function init() {
        ...
        var bindingEvents = utils.hasTouch ? touchEvents : mouseEvents;//判断是在移动端还是PC端

        //为每个事件添加监听，处理事件为handlerOriginEvent
    ...
    //touch.on=touch.bind=touch.live=_on;
